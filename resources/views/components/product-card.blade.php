...
@php
    ...
    $waText = "Halo, saya {$userName} dengan akun ({$userEmail}) ingin bertanya tentang produk ini:%0A%0A"
        . "🛍️ *{$product->name}*%0A"
        . "💰 " . $product->formatted_display_price . "%0A"
        . "📝 " . \Illuminate\Support\Str::limit(strip_tags($product->description), 100) . "%0A"
...
<div class="bg-white rounded-2xl overflow-hidden border border-gray-100 card-hover flex flex-col">
    <a href="{{ route('products.show', $product) }}" class="block relative aspect-square overflow-hidden bg-gray-100">
        ...
        @if($product->stock <= 5 && $product->stock > 0)
            <span class="absolute top-2 left-2 bg-primary text-dark text-[10px] font-bold px-2 py-1 rounded-full">Stok Terbatas</span>
        @elseif($product->stock == 0)
            <span class="absolute top-2 left-2 bg-red-500 text-white text-[10px] font-bold px-2 py-1 rounded-full">Habis</span>
        @endif
    </a>
    <div class="p-3 flex flex-col flex-1">
        ...
        @else
            @if(auth()->user()->isCustomer())
                ...
                <div class="grid grid-cols-2 gap-2">
                    <button type="button"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17"/></svg>
            ...
            @else
                ...
                <span class="w-full inline-block text-center bg-gray-100 text-gray-400 text-sm font-semibold py-2.5 rounded-xl">Mode Admin</span>
            @endif
        @endguest
    </div>
</div>
